...
anomaly_plot <- ggplot(data = avg_biomass_per_year, aes(x = year, y = biomass_anomaly)) +
  ...
  scale_fill_brewer(palette = "Set2") +
  #scale_fill_manual(values=c("Above Avg." = "steelblue", "Below Avg." = "firebrick1")) +
  geom_vline(xintercept = 5, linetype = "longdash", alpha = .8) +
  annotate("text", x = 6.05, y = 12, label = "Cyclone Oli", size = 3, alpha = .8) +
  annotate("rect", xmin = 1, xmax = 4, ymin = -Inf, ymax = Inf,
           alpha = .3,fill = "red") +
  annotate("text", x = 2.5, y = 12, label = "Crown of Thorns
Outbreak", size = 3, colour = "red", alpha = .5) +
  ...
  subtitle = "Moorea, French Polynesia (2006-2021)",
  caption = "Figure 3: Annual mean herbivorous reef fish biomass compared to the long term average
from 2006–2021 (orange and green bars), based on data from the MCR LTER, plus the
Crown of Thorns Sea Star outbreak (shaded red) and Cyclone Oli (dashed line)",
       x = "Year",
       y = "Fish Biomass Anomaly (gm^2)") +
  theme_bw() +
...
ggplot(data = avg_biomass_per_year, aes(x = year, y = mean_biomass, fill = "darkseagreen")) +
  geom_bar(stat = "identity", position = "identity") +
  guides(fill="none") +
  geom_vline(xintercept = 5, alpha = .8) +
  annotate("text", x = 6.05, y = 200, label = "Cyclone Oli", size = 3, alpha = .8) +
  annotate("rect", xmin = 1, xmax = 4, ymin = -Inf, ymax = Inf,
           alpha = .3, fill = "purple") +
...
Outbreak", size = 3, colour = "black", alpha = .8) +
  ...
  subtitle = "Moorea, French Polynesia (2006-2021)",
  caption = "Figure 3: Annual mean biomass of herbivorous reef fish in Moorea from 2006–2021 (coral-pink bars), based on 
data from the MCR LTER, plus the Crown of Thorns Sea Star outbreak (shaded purple) and Cyclone Oli (solid line)",
       x = "Year",
...
ggplot(diamonds) +
  ...
  scale_fill_fish_d(option = "Acanthurus_triostegus") +
  add_fishape(family = "Acanthuridae",
              option = "Acanthurus_triostegus",
              xmin = 1, xmax = 3, ymin = 15000, ymax = 20000,
              fill = fish(option = "Acanthurus_triostegus", n = 5)[3],
              alpha = 0.8) +
  theme_bw()
